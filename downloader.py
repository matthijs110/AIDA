...
class downloadThread (threading.Thread):
    ...
    def run(self):
        download_images(self, self.image_range, self.image_directory, self.size, self.config)

def download_images(thread, image_range, image_directory, size, config):

    numOfDownloadedImages = 0

    log.info(f"Starting download {thread.name}")
    ...
    for west_south in image_range:
        ...
        thread.totalProgessBar.increment()
        thread.updateProgressBar(numOfDownloadedImages)
        progressBar.move(status.bottomLine + 1, 0)
